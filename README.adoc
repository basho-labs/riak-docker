...
This Docker image has support for automatically creating a cluster by setting the environment variable `COORDINATOR_NODE` to the IP address of a node to which you want to join when container starts.
...
for i in 1 2; do
  CONTAINER=$(docker run -d --label=asciibuild.name="Riak in Docker" -e COORDINATOR_NODE=$COORDINATOR_NODE <% riak_tag %>)
