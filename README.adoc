...
----
# {{=<% %>=}}
SCHEMAS_CONTAINER=$(docker run -d -P --label role=cluster -v $(pwd)/test/schemas:/etc/riak/schemas <% riak_tag %>)
...
docker inspect -f '{{.State.Status}}' $SCHEMAS_CONTAINER
...
TYPE=$(echo $BUCKET_TYPES | jq -r '.bucket_types[] | select(.id == "test") | .props.datatype')
[ "$TYPE" == "counter" ]
